...
function personasFun(personas = [], empresa) {
  const arrayNombreEmpresa = []
  const filtroEmpresa = []

  personas.map(valor => {    
    //console.log(valor.nombre)
    const arrayExperiencia = valor.datos.laboral.experiencia

    arrayExperiencia.map(valor => {
      //console.log(valor.lugar)
      arrayNombreEmpresa.push(valor.lugar)      
    })

    //console.log(valor)
  })
  //console.log(arrayNombreEmpresa)
  if (!arrayNombreEmpresa.includes(empresa)) return console.log(false)

  personas.map(valor => {
    let nombrePerso = valor.nombre

    valor.datos.laboral.experiencia.map(valor => {

      if (valor.lugar === empresa) {
        filtroEmpresa.push({nombre: nombrePerso,
                            lugar: valor.lugar})
      }
    })
  ...
  if (filtroEmpresa.length === 1) {

    console.log(`La única persona que paso por esta empresa: ${filtroEmpresa[0].lugar}, es: ${filtroEmpresa[0].nombre} `)

  }
  //console.log(filtroEmpresa)
  //console.log(filtroEmpresa.length)

}

personasFun(personas,'Empresa WWW')