...
function localesHabilitadosFuncion(empresas = []) {

  return empresas.map( empresa => {

    const { nombre,
            informacion: { locales, secciones }
          } = empresa
      
    const localesEntradas = Object.values(locales)

    const localesHabilitados = []
    
    localesEntradas.map( local => {
      //console.log(local.habilitado)
      if (local.habilitado) {
        //console.log(local.especificaciones.nombre)
        localesHabilitados.push(local.especificaciones.nombre)
      } 
    })    

    const seccionesHabilitadas = secciones.map(seccion => seccion.nombre)

    const filtroSecciones = seccionesHabilitadas.filter( seccion => localesHabilitados.includes(seccion))
    //console.log(seccionesHabilitadas)
    //console.log(filtroSecciones)


    return {
      nombre,
      localesHabilitados: filtroSecciones
    }      
  })
}
...
console.log(localesHabilitadosFuncion(empresas))
