...
function personasFun (personas = [], cargo) {
  const cargos = []
  const empresas = []
  let nombre
  let sumatoria = 0
  ...
  personas.map(persona => {
      ...
      persona.datos.laboral.experiencia.map(valor => {
        //console.log(valor.cargo)
        cargos.push(valor.cargo)
      })
    
  })
 
  if (!cargos.includes(cargo)) return console.log(false)

  personas.map(persona => {

    persona.datos.laboral.experiencia.map(valor => {

      if (valor.cargo === cargo) {
        nombre = persona.nombre
        empresas.push(valor.lugar)        
        sumatoria += valor.duracion
      }      
    })   
...
personasFun(personas, 'Atención al Público')